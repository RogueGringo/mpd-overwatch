...
These modules are the core IP. They have 64 passing tests and verified references. They are not modified in this refactor — only consumed differently by the new dashboard layer.
...
| `data/las_parser.py` | LAS 2.0/3.0 parsing | CWLS standard |
| `data/edr_parser.py` | EDR format parsing | — |
| `data/models.py` | DrillingData, MeasurementPoint classes | — |
...
| `computation_log.py` | Structured .log writer — equation traces, channel decisions, topology math |
...
| `dashboard/hmu_panel.py` | Remove NPT cost reference, add [?] tooltips, drive from real loaded data |
| `dashboard/supervisory_panel.py` | Remove financial sections (NPT cost, drill savings, production value), add [?] tooltips |
...
- Validates LAS format (2.0 vs 3.0), index type (depth vs time), null value
...
Every value on the dashboard carries one of four badges:
...
Each core engine function returns an `EngineeringResult` object that carries:
- The computed value
- The method metadata (name, reference, equation string, novel flag)
- Input values with provenance tags
- Validity envelope text
- Cross-check pointers

This same object is consumed by:
- The tooltip renderer (for the [?] display)
- The computation log writer (for the .log trace)
- The report generator (for appendix sections)
- The V&V framework (to validate that tooltip equation strings match actual computation)
...
```python
def max_channels(vram_gb, cuda_cores, window=2000):
    usable = vram_gb * 0.55
    n_mem = int((usable * 1e9 / ((window + 3) * 4)) ** 0.5)
    n_comp = int(cuda_cores ** 0.4 * 10)
    return min(n_mem, n_comp, 512)  # hard cap at 512
```

| GPU | VRAM | Cores | Max Channels |
|-----|------|-------|-------------|
| RTX 3070 Ti | 8 GB | 6,144 | ~340 |
| RTX 4090 | 24 GB | 16,384 | ~512 (cap) |
| HGX-class | 48+ GB | 20,000+ | ~512 (cap) |
...
Built-in channel sets per analysis intent:

| Intent | Channel Count | Channels |
|--------|--------------|----------|
...
| Drilling Optimization | ~30 | Above minus MPD-specific, plus vibration (axial, lateral, tangential), bit_rpm, mud_motor_rpm, stick_slip, MSE_downhole, dog_leg_severity, string_torque, overpull |
| Wellbore Stability | ~28 | Core pressure/flow channels plus pit_volumes (1-7), gas_total, gas_methane, mud_temp_in, mud_temp_out, gain_loss, casing_pressure, flow_deviation |
...
2. Detect hardware (GPU model, VRAM, CUDA cores, system RAM)
...
|--------|----------------------|----------|-------|
| `.las` | 10 | **Now** | LAS 2.0 + 3.0, depth + time indexed |
